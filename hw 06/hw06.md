...
In this homework, we are going to:

-   Write two functions that do linear regression and robust regression on the Gapminder data, and compare their estimated mspe.

-   Create a nested data frame and map a linear regression function and a robust regression function over the list column holding the nested data, and make some exploration to get information out of the linear models.

-   Practice two of the exercises from the Strings chapter.
...
**(3)** use `map()` inside `mutate()`, store the model inside gap\_nested in another two list-columns.
